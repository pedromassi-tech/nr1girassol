  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <Input id="nome" required value={form.nome} onChange={e => setForm({ ...form, nome: e.target.value })} className="mt-1.5 bg-background" placeholder="Seu nome completo" />
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <Label htmlFor="email" className="text-foreground/70 text-xs font-semibold tracking-wide uppercase">E-mail</Label>
          <Input id="email" type="email" required value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} className="mt-1.5 bg-background" placeholder="seu@email.com" />
        </div>
        <div>
          <Label htmlFor="whatsapp" className="text-foreground/70 text-xs font-semibold tracking-wide uppercase">WhatsApp</Label>
          <Input id="whatsapp" type="tel" required value={form.whatsapp} onChange={e => setForm({ ...form, whatsapp: e.target.value })} className="mt-1.5 bg-background" placeholder="(11) 99999-9999" />
        </div>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <Label htmlFor="empresa" className="text-foreground/70 text-xs font-semibold tracking-wide uppercase">Empresa</Label>
          <Input id="empresa" required value={form.empresa} onChange={e => setForm({ ...form, empresa: e.target.value })} className="mt-1.5 bg-background" placeholder="Nome da empresa" />
        </div>
        <div>
          <Label htmlFor="cargo" className="text-foreground/70 text-xs font-semibold tracking-wide uppercase">Cargo</Label>
          <Input id="cargo" required value={form.cargo} onChange={e => setForm({ ...form, cargo: e.target.value })} className="mt-1.5 bg-background" placeholder="Seu cargo" />
        </div>
      </div>
      <div>
        ...
        <Textarea id="desafio" rows={3} value={form.desafio} onChange={e => setForm({ ...form, desafio: e.target.value })} className="mt-1.5 bg-background" placeholder="Conte brevemente o cenário da sua empresa..." />
      </div>
